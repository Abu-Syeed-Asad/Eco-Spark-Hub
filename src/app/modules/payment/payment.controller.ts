...
const handleStripeEvent = catchAsync(async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data = await paymentService.paymentHandler(event,);
    sendRespose(res, {
...
const specificPaymentByAdmin = catchAsync(
  async (req: Request, res: Response) => {
    const { transectionId } = req.params;

    const result = await paymentService.specificPayment(
...
const complitedAllPayment = catchAsync(async (req: Request, res: Response) => {
  ...
  sendRespose(res, {
    httpStatusCode: status.OK,
    success: true,
    message: "All completed payments retrieved successfully",
    data,
  });
});
